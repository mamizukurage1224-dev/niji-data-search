...
出演（他枠）は Holodex の mentions を使います。ただし、概要欄の「サムネ：@〇〇」「Thumbnail by」「【イラスト】」のような制作のクレジットにだけ名前があるライバーは、出演に数えません（題名や、概要欄のほかの所にも名前があれば出演のまま。概要欄は見分けるのに使うだけで保存しません）。外した件数と例は実行の記録に出ます。
