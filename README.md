...
- `radar/{channel_id}.json` … ライバー別の他枠出演（`upcoming`／`past`）と自枠（`own_upcoming`／`own_past`）。過去は180日分（取っていない古い分は、毎回1000本ずつさかのぼって取る）。各項目の `kind` は 配信（`live`）・動画（`video`）・ショート（`short`）の推定（Holodex に区別が無いため、開始時刻と長さ・題名から推定）
